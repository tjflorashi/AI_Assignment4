...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        #single int - distance estimated to ghost via manhattan
        emissionModel = busters.getObservationDistribution(noisyDistance)
        #{1:p, 2:p, ... 10:p}    {truedistance: Pr(noisyDistance | Truedistance)}
        ...
        if noisyDistance == None:
            for p in self.legalPositions:
                allPossible[p] = 0 # initialize
            allPossible[self.getJailPosition()] = 1
        else:
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition) # Pr(et | Xt)
                #  Given in problem, Pr(noisyDistance | Xt=p) =
                #  Pr(noisyDistance | manhattanDistance(p,packmanPosition))
                allPossible[p] = self.beliefs[p] * emissionModel[trueDistance]
                # emissionModel[trueDistance] = Pr(noisyDistance | trueDistance)  = Pr (et | Xt=p)
                # Pr(Xt | et,...,e1) = normalize(Pr(Xt|et-1,...,e1)Pr(et|Xt) = normalize(previousbelief * emissionmodel[truedist])
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        max_count = self.numParticles
        self.particles = []

        while max_count > len(self.legalPositions):
            self.particles.extend(self.legalPositions)
            max_count = max_count - len(self.legalPositions)
        # when max_count <= len(self.legalPositions)
        self.particles.extend(self.legalPositions[0:max_count-1])
        "*** END YOUR CODE HERE ***"
    ...
    def observe(self, observation, gameState):
        ...
        # update all particles when ghost is captured by pacman
        if noisyDistance == None:
            for i in range(0, len(self.particles)):
                self.particles[i] = self.getJailPosition()
        # update belief with weight P(e_t|Xt=p)
        else:
            for p, prob in newBelief.items(): # p ghost position
                trueDistance = util.manhattanDistance(pacmanPosition, p)
                newBelief[p] = prob * emissionModel[trueDistance]

            # check whether all particles receive 0 weight
            if newBelief.totalCount() == 0:
                self.initializeUniformly(gameState)
            # resample particles based on belief distribution
            else:
                # normalize the new belief distribution
                newBelief.normalize()
                for j in range(0, len(self.particles)):
                    self.particles[j] = util.sample(newBelief)
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPostDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while (self.numParticles - count) > len(permutations):
            self.particles.extend(permutations)
            count += len(permutations)

        # add the particles to the list when (self.numParticles - count) <= len(permutations)
        self.particles.extend(permutations[0: (self.numParticles - count)])
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        ghostsCaptured = []
        previousBelief = self.getBeliefDistribution()  # previous belief with keys are not updated
        newBelief = util.Counter()

        # update the self.particles list by checking whether ghosts are captured by pacman and store the captured ghost
        # index in a list
        for i in range(self.numGhosts):
            if noisyDistances[i] == None:
                ghostsCaptured.append(i)
                for j in range(self.numParticles):
                    self.particles[j] = self.getParticleWithGhostInJail(self.particles[j], i) # a tuple o fall ghosts

        # check whether all ghosts are captured, if not do the following
        if len(ghostsCaptured) < self.numGhosts:
            # create a new belief and store the corresponding values
            for particle, prob in previousBelief.items():
                updatedParticle = particle
                updatedProb = prob
                for g in range(self.numGhosts):
                    if noisyDistances[g] == None:
                        updatedParticle = self.getParticleWithGhostInJail(particle, g)
                    else:
                        trueDistance = util.manhattanDistance(pacmanPosition, particle[g])
                        updatedProb = updatedProb * emissionModels[g][trueDistance]
                newBelief[updatedParticle] += updatedProb

            # check if all particles received with weight 0
            if newBelief.totalCount() == 0:
                self.initializeParticles()
                for k in ghostsCaptured:
                    for s in range(self.numParticles):
                        self.particles[s] = self.getParticleWithGhostInJail(self.particles[s], k)
            # resample particles based on belief distribution
            else:
                # normalize the new belief distribution
                newBelief.normalize()
                for l in range(self.numParticles):
                    self.particles[l] = util.sample(newBelief)
    ...
    def elapseTime(self, gameState):
        ...
        #i.e.newParticle: [(2,3), (14,3)]      when there are two ghosts, G1 pos, G2 pos
            "*** YOUR CODE HERE ***"
            # setGhostPositions returns a gameState
            for ghostIndex in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(
                    setGhostPositions(gameState, newParticle), ghostIndex, self.ghostAgents[ghostIndex]
                )
